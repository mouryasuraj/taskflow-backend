...
export const validateUpdateTaskReqBody = (req, next) => {
  if (!req.params.id) return next(new AppError("task id is required", 400));
  if (!req?.body || Object.keys(req?.body || {}).length === 0)
    return next(new AppError(reqBodyNotPresentTxt, 400));
  ...
  const extraFields = reqBodyFields.filter(
    (f) => !allowedUpdateTaskFields.includes(f),
  );
  if (extraFields.length > 0) {
    const errorMsg = `fields are not allowed: [${extraFields.join(", ")}]`;
    return next(new AppError(errorMsg, 400));
  ...
  const isMissingFields = !allowedUpdateTaskFields.every((f) =>
    reqBodyFields.includes(f),
  );
  if (isMissingFields) {
    const errorMsg = `Required fields are missing: ${allowedUpdateTaskFields.join(", ")}`;
    return next(new AppError(errorMsg, 400));
  }
...
export const validateDeleteTask = (req, next) => {
  const { id } = req.params;
  const { project_id } = req.query;
  if (!id) {
    logError(`task id is required`, `task id is required`);
    return next(new AppError("task id is required", 400));
  }
  if (!project_id) {
    logError(`project_id is required`, `project_id is required`);
