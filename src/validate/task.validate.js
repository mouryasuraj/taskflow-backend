import { allowedTaskFields, allowedTaskPriority, allowedTaskStatus, AppError, isNotValid, isRequired, reqBodyNotPresentTxt } from "../utils/index.js"
...
export const validateTaskReqBody = (req, next) =>{
    if (!req?.body || Object.keys(req?.body || {}).length === 0) return next(new AppError(reqBodyNotPresentTxt, 400))
        ...
        const extraFields = reqBodyFields.filter(f => !allowedTaskFields.includes(f))
        if (extraFields.length > 0) {
            const errorMsg = `fields are not allowed: [${extraFields.join(", ")}]`
            return next(new AppError(errorMsg, 400))
        }
    
        const isMissingFields = !allowedTaskFields.every(f => reqBodyFields.includes(f))
        if (isMissingFields) {
        ...
        const taskFieldValidation = [
            ...
            { field:"status", isValid: status && !allowedTaskStatus.includes(status), message: isNotValid },
            { field:"priority", isValid: !priority.trim(), message: isRequired },
            { field:"priority", isValid: priority && !allowedTaskPriority.includes(priority), message: isNotValid },
